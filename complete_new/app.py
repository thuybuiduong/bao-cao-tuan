...
def full_guard(path):
    ...
    required=[
        'I. TÌNH HÌNH TỘI PHẠM VÀ VI PHẠM',
        'II. CÔNG TÁC CÔNG TỐ, KIỂM SÁT HOẠT ĐỘNG TƯ PHÁP',
        'III. CÔNG TÁC KHÁC',
        'IV. NHIỆM VỤ CÔNG TÁC TRỌNG TÂM TUẦN SAU',
        '3.2. Công tố và kiểm sát xét xử phúc thẩm',
        '4.2. Kiểm sát thi hành án hình sự',
        '5. Công tác kiểm sát việc giải quyết vụ, việc dân sự',
        '8. Công tác kiểm sát thi hành án dân sự',
        '9. Công tác giải quyết khiếu nại, tố cáo',
    ]
    return [x for x in required if x.lower() not in text.lower()]

@app.get('/health')
def health():
    return {'ok':valid_docx(TEMPLATE),'version':'complete-new-v1-template-first','inputs':3,'template_first':True,'full_sections_guard':True,'red_excel_numbers':True,'footnote_superscript':True}
...
def index():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td0:
            ...
            missing=full_guard(out)
            if missing: raise RuntimeError('Thiếu phần bắt buộc: '+ '; '.join(missing))
            data=out.read_bytes()
