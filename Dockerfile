...
COPY fallback_template.py /app/fallback_template.py
RUN python -c "from fallback_template import ensure_template; ensure_template('/app/report_template.docx')" && python -c "import zipfile; z=zipfile.ZipFile('/app/report_template.docx'); assert 'word/document.xml' in z.namelist()"
